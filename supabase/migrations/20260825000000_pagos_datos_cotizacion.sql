-- ============================================================================
-- Pagos PayPal: datos de la cotización visibles en el portal
-- ============================================================================
-- El historial de pagos mostraba solo fecha/referencia/monto: para un pago
-- que nace de una cotización de HubSpot eso es demasiado pobre — el cliente
-- ve un capture id de PayPal y nada que le diga QUÉ pagó.
--
-- Los datos se COPIAN a `public.pagos` en vez de leerse del esquema
-- `payments` en tiempo de consulta, por tres razones:
--   1. `payments` no está expuesto a PostgREST para el rol `authenticated`
--      del portal; exponerlo implicaría abrir tablas de otro proyecto y
--      escribirles RLS.
--   2. `payments.*` no tiene `cliente_id`, así que el modelo de seguridad
--      del portal (cliente_id -> perfiles) no aplica ahí.
--   3. Un libro de pagos debe guardar SNAPSHOT: si mañana editan la
--      cotización, el pago histórico no puede cambiar retroactivamente.
--      Se guarda la versión efectivamente pagada.
-- ============================================================================


-- ============================================================================
-- 1. Columnas de snapshot en `pagos`
-- ============================================================================
-- Todas nullable: los pagos manuales/CSV no tienen cotización asociada.
-- `deal_url` la arma n8n completa, para no hardcodear el portal id de
-- HubSpot en la base.

alter table public.pagos
  add column if not exists cotizacion_numero   text,
  add column if not exists deal_nombre         text,
  add column if not exists cotizacion_subtotal numeric(12, 2),
  add column if not exists cotizacion_total    numeric(12, 2),
  add column if not exists deal_url            text;

comment on column public.pagos.cotizacion_numero is
  'Número de cotización (payments.paypal_quote_payments.quote_number). Snapshot al momento del pago.';
comment on column public.pagos.deal_url is
  'Link al Deal en HubSpot. Solo para el panel admin, no se muestra al cliente.';


-- ============================================================================
-- 2. RPC: registrar_pago_paypal recibe y guarda esos datos
-- ============================================================================
-- Cambia la cantidad de parámetros, así que hay que DROP antes: un
-- `create or replace` con distinta aridad crearía una sobrecarga en vez de
-- reemplazar, y quedarían dos versiones vivas.
--
-- Los 5 parámetros nuevos van al final y son opcionales: W4 los manda, pero
-- una llamada sin ellos (o un pago sin cotización) sigue siendo válida.
-- PostgREST llama por nombre, así que el orden no rompe nada del lado de n8n.

drop function if exists public.registrar_pago_paypal(text, text, text, numeric, text, timestamptz, text, integer);
...
create or replace function public.registrar_pago_paypal(
  ...
  p_cotizacion_subtotal numeric default null,
  p_cotizacion_total    numeric default null,
  p_deal_url            text    default null
) returns table (
  cliente_id      uuid,
  es_nuevo        boolean,
...
begin
  ...
  insert into public.pagos (
    ...
    cotizacion_numero, deal_nombre, cotizacion_subtotal, cotizacion_total, deal_url
  )
  values (
    v_cliente.id, p_monto, p_fecha, 'paypal', 'paypal', btrim(p_capture_id),
    nullif(btrim(p_cotizacion_numero), ''), nullif(btrim(p_deal_nombre), ''),
    p_cotizacion_subtotal, p_cotizacion_total, nullif(btrim(p_deal_url), '')
  );
...
revoke all on function public.registrar_pago_paypal(text, text, text, numeric, text, timestamptz, text, integer, text, text, numeric, numeric, text) from public;
revoke all on function public.registrar_pago_paypal(text, text, text, numeric, text, timestamptz, text, integer, text, text, numeric, numeric, text) from anon;
revoke all on function public.registrar_pago_paypal(text, text, text, numeric, text, timestamptz, text, integer, text, text, numeric, numeric, text) from authenticated;
grant execute on function public.registrar_pago_paypal(text, text, text, numeric, text, timestamptz, text, integer, text, text, numeric, numeric, text) to service_role;

comment on function public.registrar_pago_paypal(text, text, text, numeric, text, timestamptz, text, integer, text, text, numeric, numeric, text) is
