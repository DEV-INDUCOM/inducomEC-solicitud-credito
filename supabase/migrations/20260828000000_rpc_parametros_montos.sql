-- ============================================================================
-- registrar_pago_paypal: parámetros con el mismo nombre que las columnas
-- ============================================================================
--   p_monto            -> p_monto_pagado
--   p_cotizacion_total -> p_monto_cotizado
--
-- Quedaban con el nombre viejo desde antes del renombrado de columnas
-- (20260827000000), lo que obligaba a recordar dos vocabularios para lo
-- mismo. Ahora parámetro y columna se llaman igual.
--
-- Hace falta DROP + CREATE: `create or replace function` NO permite cambiar
-- el nombre de un parámetro de entrada (falla con "cannot change name of
-- input parameter"). Los TIPOS no cambian, así que la firma para el drop es
-- la misma lista de tipos.
--
-- El DROP también borra los GRANT, por eso se vuelven a aplicar al final.
--
-- ⚠️  Impacto en n8n (W4): NINGUNO. El nodo Postgres llama por POSICIÓN
--     (`select * from registrar_pago_paypal($1, $2, ... $12)`), no por
--     nombre, y ni el orden ni los tipos de los parámetros cambian.
-- ============================================================================

drop function if exists public.registrar_pago_paypal(text, text, text, numeric, text, timestamptz, text, integer, text, text, numeric, text);
...
create or replace function public.registrar_pago_paypal(
  ...
  p_monto_cotizado    numeric default null,
...
begin
  ...
  values (
    v_cliente.id, p_monto_pagado, p_fecha, 'paypal', 'paypal', btrim(p_capture_id),
    nullif(btrim(p_cotizacion_numero), ''), nullif(btrim(p_deal_nombre), ''),
    p_monto_cotizado, nullif(btrim(p_cotizacion_url), '')
  );
...
-- El DROP de arriba borró los permisos: hay que volver a ponerlos.
-- Solo service_role (n8n): Postgres da EXECUTE a PUBLIC por defecto y
-- PostgREST expone toda función de `public` en /rest/v1/rpc/, alcanzable con
-- la anon key del navegador. Sin este revoke, cualquiera podría acuñar
-- clientes y códigos de invitación válidos.
...
--     from pg_proc where proname = 'registrar_pago_paypal';
--
--   select grantee, privilege_type
--     from information_schema.routine_privileges
--    where routine_schema = 'public' and routine_name = 'registrar_pago_paypal';
--   -- debe salir solo postgres (owner) y service_role
